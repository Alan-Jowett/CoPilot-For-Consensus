...
"""
    ...
    def _add_gcp_extensions(self) -> dict[str, Any]:
        ...
        spec["x-google-endpoints"] = [{"name": "copilot-api.endpoints.PROJECT_ID.cloud.goog", "allowCors": True}]
        ...
        for path, methods in spec.get("paths", {}).items():
            for method, operation in methods.items():
                ...
                operation["x-google-backend"] = {"address": backend_url, "deadline": 30.0}
    ...
    def _generate_cloud_run_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "namespace": "default",
                "labels": {"app": "copilot-for-consensus", "component": "gateway"},
            },
            "spec": {
                "template": {
                    "metadata": {
                        ...
                        "containers": [
                            {
                                ...
                                "env": [
                                    ...
                                    {"name": "ESPv2_ARGS", "value": "--cors_preset=basic --cors_allow_origin=*"},
                                ],
    ...
    def _generate_k8s_config(self) -> list:
        ...
        return [
            ...
            },
            {
                ...
                "spec": {
                    "replicas": 2,
                    "selector": {"matchLabels": {"app": "copilot-gateway"}},
                    "template": {
                        "metadata": {"labels": {"app": "copilot-gateway"}},
                        "spec": {
                            "containers": [
                                {
                                    "name": "esp",
    ...
    def _generate_esp_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "backend": {"address": "http://backend-service:8080", "timeout": 30},
            "cors": {
                "enabled": True,
                "allow_origins": ["*"],
