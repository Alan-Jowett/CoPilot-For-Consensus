...
This service uses **pip-tools** for dependency management with lockfiles for repeatable builds and better security tracking.
...
- **Local adapters**: Installed separately in Dockerfile and not included in lockfiles

### Updating Dependencies

To add or update a dependency:

1. Edit `requirements.in` with the new or updated dependency
...
- **Repeatable builds**: Exact versions locked across all environments
- **Security**: Transitive dependencies (like `starlette` via `fastapi`) are explicitly tracked
- **Supply chain protection**: All dependency changes are visible in PRs
- **Dependabot integration**: Security updates for transitive dependencies appear in Dependabot PRs

### CI Validation

The CI pipeline validates that `requirements.txt` is in sync with `requirements.in`. If you see a validation failure:
...
For more details, see [CONTRIBUTING.md](../CONTRIBUTING.md#dependency-management).
