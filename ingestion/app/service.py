...
from copilot_message_bus import ArchiveIngestedEvent, ArchiveIngestionFailedEvent, ArchiveMetadata, EventPublisher
...
class IngestionService:
    ...
    def __init__(
        ...
        self.logger = logger or create_logger(
            "stdout",
            {"level": "INFO", "name": "ingestion"},
        )
        self.metrics = metrics or create_metrics_collector(driver_name="noop")
            ...
            try:
                ...
                if error_reporter_adapter is not None:
                    ...
                    if adapter_driver_config is None:
                        normalized_driver_config: dict[str, Any] = {}
                    elif hasattr(adapter_driver_config, "config"):
                        normalized_driver_config = dict(getattr(adapter_driver_config, "config"))  # type: ignore[arg-type]
                    else:
                        normalized_driver_config = dict(adapter_driver_config)  # type: ignore[arg-type]

                    self.error_reporter = create_error_reporter(
                        driver_name=error_reporter_adapter.driver_name,
                        driver_config=normalized_driver_config,
                    )
                else:
                    self.error_reporter = create_error_reporter(driver_name="silent")
            except Exception:
                self.error_reporter = create_error_reporter(driver_name="silent")
        else:
