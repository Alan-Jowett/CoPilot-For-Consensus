...
class TestIngestionService:
    ...
    def service(self, config):
        ...
        logger = create_logger(driver_name="silent", driver_config={"level": "INFO", "name": "ingestion-test"})
    ...
    def test_error_reporter_integration(self, temp_storage):
        ...
        publisher = create_publisher(driver_name="noop")
        publisher.connect()
        error_reporter = create_error_reporter(driver_name="silent")
        logger = create_logger(driver_name="silent", driver_config={"level": "INFO", "name": "ingestion-test"})
        metrics = create_metrics_collector(driver_name="noop")
...
=======
>>>>>>> 5092fbf (refactor: enforce schema-driven DriverConfig-only configuration across all adapters)
    config = make_config(storage_path=str(tmp_path))
    publisher = create_publisher(driver_name="noop")
    publisher.connect()
...
>>>>>>> 5092fbf (refactor: enforce schema-driven DriverConfig-only configuration across all adapters)
    ...
    document_store = create_document_store(driver_name="inmemory", strict_validation=False)
    document_store.connect()
