...
- **Documentation:** See [retry policy](../operations/retry-policy.md) for the complete specification and operational procedures.
...
- **Centralized Schema Definitions**: Each service has a JSON schema file in `docs/schemas/configs/<service>.json` defining all configuration fields
...
See [Schema-Driven Configuration Migration](../operations/configuration-migration.md) for detailed migration instructions for existing services.
...
3. **ChunksPrepared** (Chunking → Message Bus)
  - Payload: `{archive_id, chunk_ids, chunk_count, chunks_ready, timestamp}`
