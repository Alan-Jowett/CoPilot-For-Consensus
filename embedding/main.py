...
logger = create_logger("stdout", {"level": "INFO", "name": "embedding"})
...
def main():
    ...
    try:
        ...
        subscriber_driver_config["queue_name"] = "chunks.prepared"
        ...
        vector_store_driver_config = dict(vector_store_adapter.driver_config.config)
        ...
        embedding_driver_config = dict(embedding_adapter.driver_config.config)

        backend_name = str(embedding_adapter.driver_name).lower()
        provider_driver_name = backend_name
        provider_config = embedding_driver_config

        embedding_provider = create_embedding_provider(
            driver_name=provider_driver_name,
            driver_config=provider_config,
        )
        ...
        if metrics_adapter is not None:
            ...
            metrics_driver_config.setdefault("job", "embedding")
