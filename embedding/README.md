...
The Embedding Service subscribes to the following events. See [SCHEMA.md](../docs/schemas/data-storage.md#message-bus-event-schemas) for complete event schemas.
...
See [ChunksPrepared schema](../docs/schemas/data-storage.md#5-chunksprepared) in SCHEMA.md for the complete payload definition.
...
The Embedding Service publishes the following events. See [SCHEMA.md](../docs/schemas/data-storage.md#message-bus-event-schemas) for complete event schemas.
...
See [EmbeddingsGenerated schema](../docs/schemas/data-storage.md#7-embeddingsgenerated) in SCHEMA.md for the complete payload definition.
...
See [EmbeddingGenerationFailed schema](../docs/schemas/data-storage.md#8-embeddinggenerationfailed) in SCHEMA.md for the complete payload definition.
...
The Embedding Service automatically requeues chunks without embeddings on startup to ensure forward progress after service crashes or restarts.

### Behavior

On service startup (before subscribing to events), the service:
1. Queries the document store for chunks with `embedding_generated: false`
2. Publishes `ChunksPrepared` events for each chunk without embeddings
...
- **Query Limit**: Requeues up to 1000 chunks per startup
