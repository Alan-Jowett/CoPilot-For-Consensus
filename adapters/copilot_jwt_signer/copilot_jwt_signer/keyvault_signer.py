...
class KeyVaultJWTSigner(JWTSigner):
    ...
    def _initialize_clients(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Initialized Key Vault JWT signer for %s with key %s in %s",
                self.algorithm, self.key_name, self.key_vault_url
            )
            ...
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    if attempt < self.max_retries and self._is_transient_error(e):
                        ...
                        logger.warning(
                            "Key Vault sign operation failed (attempt %d/%d), retrying in %.1fs: %s",
    ...
    def _hash_message(self, message: bytes) -> bytes:
        ...
        if self.algorithm in ("RS384", "ES384"):
            return hashlib.sha384(message).digest()
        if self.algorithm in ("RS512", "ES512"):
