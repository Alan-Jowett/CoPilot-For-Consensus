...
def test_preserve_extra_flag_with_aggregations(store: DocumentStore):
    ...
    pipeline = [
        {"$lookup": {"from": "chunks", "localField": "message_id", "foreignField": "message_id", "as": "chunks"}}
    ]
...
def test_schema_with_empty_properties():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        empty_schema = {"$schema": "http://json-schema.org/draft-07/schema#", "type": "object", "properties": {}}
...
def test_missing_schema_file_for_collection():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        archives_schema = {
            "$schema": "http://json-schema.org/draft-07/schema#",
            "type": "object",
            "properties": {"_id": {"type": "string"}, "file_hash": {"type": "string"}},
        }
...
def test_aggregate_with_computed_fields_preserved(store: DocumentStore):
    ...
    pipeline = [{"$match": {"summary_id": None}}]
