...
@pytest.mark.asyncio
async def test_store_archive(store, temp_dir):
...
async def test_get_archive(store):
    ...
    retrieved = await store.get_archive(archive_id)
...
async def test_get_archive_not_found(store):
    ...
    result = await store.get_archive("nonexistent_id")
    assert result is None


@pytest.mark.asyncio
async def test_get_archive_by_hash(store):
    ...
    # Look up by hash
    found_id = await store.get_archive_by_hash(content_hash)
    assert found_id == archive_id


@pytest.mark.asyncio
async def test_get_archive_by_hash_not_found(store):
    """Test hash lookup for non-existent content."""
    result = await store.get_archive_by_hash("0" * 64)
    assert result is None


@pytest.mark.asyncio
async def test_archive_exists(store):
    ...
    # Should exist
    assert await store.archive_exists(archive_id) is True
    
    # Non-existent should not exist
    assert await store.archive_exists("nonexistent") is False


@pytest.mark.asyncio
async def test_delete_archive(store, temp_dir):
    ...
    assert await store.archive_exists(archive_id) is False
...
@pytest.mark.asyncio
async def test_delete_archive_not_found(store):
    """Test deleting non-existent archive returns False."""
    result = await store.delete_archive("nonexistent")
    assert result is False


@pytest.mark.asyncio
async def test_list_archives(store):
    ...
    id1 = await store.store_archive("source-a", "archive1.mbox", content1)
    id2 = await store.store_archive("source-a", "archive2.mbox", content2)
    id3 = await store.store_archive("source-b", "archive3.mbox", content3)
    
    # List archives for source-a
    archives_a = await store.list_archives("source-a")
    ...
    archives_b = await store.list_archives("source-b")
...
async def test_list_archives_empty(store):
    ...
    archives = await store.list_archives("nonexistent-source")
    assert archives == []
...
async def test_deduplication(store):
    ...
    id1 = await store.store_archive("source-a", "file1.mbox", content)
    id2 = await store.store_archive("source-b", "file2.mbox", content)
...
@pytest.mark.asyncio
async def test_metadata_persistence(temp_dir):
    ...
    archive_id = await store1.store_archive("test-source", "test.mbox", content)
    ...
    retrieved = await store2.get_archive(archive_id)
    assert retrieved == content
    
    # Should appear in listings
    archives = await store2.list_archives("test-source")
    assert len(archives) == 1
    assert archives[0]["archive_id"] == archive_id


@pytest.mark.asyncio
async def test_archive_metadata_structure(store):
    """Test that archive metadata has correct structure."""
    content = b"Test content"
    archive_id = await store.store_archive("test-source", "test.mbox", content)
    
    archives = await store.list_archives("test-source")
    assert len(archives) == 1
