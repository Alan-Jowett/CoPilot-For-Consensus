...
# Copilot Error Reporting Adapter
...
```bash
cd adapters/copilot_error_reporting
pip install -e ".[dev]"
```
...
```bash
pip install copilot-error-reporting
```

### With Sentry Support

```bash
pip install copilot-error-reporting[sentry]
```
...
```python
from copilot_error_reporting import ConsoleErrorReporter
...
```python
from copilot_error_reporting import SilentErrorReporter
...
```python
from copilot_error_reporting import SentryErrorReporter
...
```bash
pytest tests/ --cov=copilot_error_reporting --cov-report=html
```

### Linting

```bash
pylint copilot_error_reporting/
```
