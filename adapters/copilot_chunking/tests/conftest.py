...
"""Pytest config for copilot_chunking adapter tests.

These adapter tests are often executed from within the adapter folder.
In that mode, sibling adapters (like `copilot_config` and
`copilot_schema_validation`) are not automatically on `sys.path`.
...
def pytest_configure() -> None:
    ...
    _add_adapter_to_syspath(repo_root, "copilot_schema_validation")
