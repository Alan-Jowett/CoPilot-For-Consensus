        ...
        try:
            ...
            if not policy.should_retry(context, e):
                ...
                raise RetryExhaustedError(
                    f"Retry exhausted after {context.attempt_number} attempts", context, dlq_info
                ) from e

            # Calculate delay and sleep
            delay_ms = policy.calculate_delay_ms(context.attempt_number + 1)

            logger.warning(f"Retryable error on attempt {context.attempt_number}, " f"retrying in {delay_ms}ms: {e}")
            ...
            logger.error(f"Non-retryable error on attempt {context.attempt_number}: {e}", exc_info=True)
