    ...
    def _validate_event(self, event: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if schema is None:
                ...
                return not self._strict, [] if not self._strict else [f"No schema found for event type '{event_type}'"]
        except Exception as exc:
            logger.error(f"Failed to retrieve schema for event type '{event_type}': {exc}")
    ...
    def publish(self, exchange: str, routing_key: str, event: Dict[str, Any]) -> bool:
        ...
        if not is_valid:
            ...
            if self._strict:
                ...
                logger.warning(
                    f"Event validation failed for '{event_type}' but continuing "
                    f"in non-strict mode: {errors}"
                )
