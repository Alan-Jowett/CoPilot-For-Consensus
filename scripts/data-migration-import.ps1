...
elseif ($DestType -eq "cosmos") {
    ...
    elseif ($env:DST_COSMOS_ENDPOINT -and $env:DST_COSMOS_KEY) {
        ...
        # since mongoimport requires a MongoDB API account.
        $endpointUri = [Uri]$env:DST_COSMOS_ENDPOINT
        $host = $endpointUri.Host

        if ($host -like "*.documents.azure.com") {
            throw "DST_COSMOS_ENDPOINT '$host' appears to be a SQL API endpoint. This script requires a Cosmos DB MongoDB API account. Use -ConnectionString with a MongoDB connection string, or use the Python import script (data-migration-import.py) for SQL API accounts."
        }
