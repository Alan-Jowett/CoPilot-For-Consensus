...
def test_allowlist_entire_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        is_allowed = is_allowlisted(test_file, 'value = os.environ.get("VAR")', allowlist, tmpdir_path)
        assert is_allowed
...
def test_allowlist_with_regex():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        is_allowed = is_allowlisted(test_file, 'value = os.environ.get("ALLOWED_VAR")', allowlist, tmpdir_path)
        assert is_allowed

        # Check that line without the pattern is not allowlisted
        is_not_allowed = is_allowlisted(test_file, 'value = os.environ.get("DIFFERENT_VAR")', allowlist, tmpdir_path)
