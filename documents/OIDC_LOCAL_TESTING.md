...
```bash
export AUTH_GITHUB_CLIENT_ID="your_client_id_from_step_1"
```

Or add to `.env` for `docker compose`:

```env
AUTH_GITHUB_CLIENT_ID=your_client_id_from_step_1
```
...
Create a `.env` file in the repo root (or update your existing one) with all OIDC settings:
...
# Microsoft OAuth
AUTH_MS_CLIENT_ID=your_microsoft_client_id
AUTH_MS_TENANT=common
...
**Important**: Add `.env` to `.gitignore` if not already present—it contains sensitive client IDs (though not secrets, it's good practice).
...
- [ ] All three client secrets stored in `./secrets/`
- [ ] All three client IDs set in environment (`.env` or shell)
...
- Double-check that the client ID in your env var matches the one from the provider
- Verify the secret file contents (`cat secrets/github_oauth_client_secret`)
- Rebuild auth after changing env: `docker compose build auth`

### Auth service not picking up env vars
- Ensure `docker compose build auth` was run after updating `.env`
- If using shell exports, verify with `echo $AUTH_GITHUB_CLIENT_ID` before starting docker
