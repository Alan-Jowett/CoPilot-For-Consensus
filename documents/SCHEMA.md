...
**Breaking Change (v1.1):** The `prompt_template` field now contains the full concatenated prompt text (system instructions + user template) instead of a template identifier string. This enables dynamic prompt customization and KV cache optimization in LLM backends. The full prompt is generated by the Orchestration Service at event publish time by merging:
- `system.txt`: Static system instructions for summarization (cached in LLM KV cache)
- `user.txt`: Variable user message template with placeholders for thread-specific data
...
**Fields:**
- `thread_ids` (Array[String]): Thread identifiers to summarize
- `top_k` (Integer): Number of top-k chunks retrieved for context
- `llm_backend` (String): LLM backend identifier (ollama, azure, openai)
- `llm_model` (String): Model name/identifier
- `context_window_tokens` (Integer): Token budget for context window
- `prompt_template` (String): Full concatenated system + user prompt text. Previously was a template identifier; now contains complete prompt instructions and variable placeholders.
- `chunk_count` (Integer): Number of chunks provided in context
- `message_count` (Integer): Number of unique messages in context
