...
"""Integration tests for the summarization service API."""
...
def mock_document_store():
    ...
    store = Mock()
    store.insert_document = Mock(return_value="summary_123")
...
def client(test_service, monkeypatch):
    ...
    monkeypatch.setattr(main, "summarization_service", test_service)
...
def test_readyz_endpoint_service_ready(client, test_service, monkeypatch):
    ...
    assert data["service"] == "summarization"
...
def test_readyz_endpoint_service_not_initialized(client, monkeypatch):
    ...
    # Set service to None
    monkeypatch.setattr(main, "summarization_service", None)
