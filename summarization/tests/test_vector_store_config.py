...
class TestVectorStoreConfiguration:
    ...
    def test_azure_ai_search_required_fields_validation(self):
        ...
        # Missing endpoint
        config = MockConfig(
            vector_store_type="azure_ai_search", embedding_dimension=384, aisearch_index_name="test-index"
        )
    ...
    def test_faiss_embedding_dimension_validation(self):
        ...
        config = MockConfig(vector_store_type="faiss", vector_store_index_type="flat")

        assert not hasattr(config, "embedding_dimension")

        # Zero embedding_dimension
        config = MockConfig(vector_store_type="faiss", embedding_dimension=0, vector_store_index_type="flat")

        assert hasattr(config, "embedding_dimension")
        assert config.embedding_dimension == 0

        # Negative embedding_dimension
        config = MockConfig(vector_store_type="faiss", embedding_dimension=-1, vector_store_index_type="flat")

        assert config.embedding_dimension < 0

        # Valid embedding_dimension
        config = MockConfig(vector_store_type="faiss", embedding_dimension=384, vector_store_index_type="flat")
    ...
    def test_inmemory_backend_no_validation(self):
        ...
        config = MockConfig(vector_store_type="inmemory")
    ...
    def test_unsupported_backend_error_message(self):
        ...
        config = MockConfig(vector_store_type="unsupported_backend")
...
class TestAzureAISearchConfiguration:
    ...
    def test_endpoint_validation(self):
        ...
        # Empty endpoint
        config = MockConfig(aisearch_endpoint="")
        assert not config.aisearch_endpoint

        # Valid endpoint
        config = MockConfig(aisearch_endpoint="https://test.search.windows.net")
        assert config.aisearch_endpoint

    def test_index_name_validation(self):
        """Test index_name validation logic."""
        # Empty index_name
        config = MockConfig(aisearch_index_name="")
        assert not config.aisearch_index_name

        # Valid index_name
        config = MockConfig(aisearch_index_name="document-embeddings")
        assert config.aisearch_index_name

    def test_dimension_validation(self):
        """Test embedding_dimension validation for Azure AI Search."""
        # Zero dimension
        config = MockConfig(embedding_dimension=0)
        assert config.embedding_dimension <= 0

        # Negative dimension
        config = MockConfig(embedding_dimension=-384)
        assert config.embedding_dimension <= 0
    ...
    def test_api_key_vs_managed_identity(self):
        ...
        config = MockConfig(aisearch_use_managed_identity=True)
        api_key = getattr(config, "aisearch_api_key", None)
        assert config.aisearch_use_managed_identity is True
        assert api_key is None

        # API key provided
        config = MockConfig(aisearch_api_key="test-key-123", aisearch_use_managed_identity=False)
