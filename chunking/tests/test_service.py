...
def test_idempotent_chunk_insertion_cosmosdb(chunking_service, mock_document_store, mock_publisher):
    """Test that duplicate chunk insertions from CosmosDB are handled gracefully (idempotency)."""
    from copilot_storage import DocumentStoreError

    # Setup mock to simulate CosmosDB duplicate on second insert
    messages = [
        {
            "_id": "abc123def4567890",
            "message_id": "<test@example.com>",
    ...
    # First insert succeeds, second raises CosmosDB-style DocumentStoreError
    insert_count = [0]

    def insert_side_effect(collection, document):
        insert_count[0] += 1
        if insert_count[0] == 2:
            # Simulate the CosmosDB error from azure_cosmos_document_store.py
            raise DocumentStoreError(
                f"Document with id {document.get('_id', 'unknown')} already exists in collection {collection}"
            )
        return f"chunk_{insert_count[0]}"
...
def test_requeue_publishes_schema_compliant_jsonparsed_events():
    """Test that startup requeue publishes schema-compliant JSONParsed events.
    
    This validates that requeued events include all required fields per the
    JSONParsed.schema.json specification, including thread_count, thread_ids,
    and parsing_duration_seconds.
    """
    from copilot_config.generated.adapters.document_store import (
        AdapterConfig_DocumentStore,
        DriverConfig_DocumentStore_Inmemory,
    ...
    # Canonical hex IDs
    archive_id = "1111111111111111"
    msg_id = "aaaaaaaaaaaaaaaa"

    # Insert test data: 1 message without chunks
    store.insert_document("messages", {"_id": msg_id, "archive_id": archive_id, "body_normalized": "test message"})
    ...
    # Validate against JSONParsed schema
    assert_valid_event_schema(event)
    
    # Additional verification of required fields
    event_data = event["data"]
    assert event_data["archive_id"] == archive_id
    assert msg_id in event_data["message_doc_ids"]
    assert event_data["message_count"] == 1
    assert "thread_count" in event_data
    assert "thread_ids" in event_data
    assert "parsing_duration_seconds" in event_data
